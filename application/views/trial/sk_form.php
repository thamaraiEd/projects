 				...
 				<div id="successmsg" style="color: green; font-size:20px; font-weight:600; text-align: center;"></div>		 
                    ...
                    <div class="form-group">
						...
						<div class="row"> 
							...
							<div class="col-md-6">
								<label for="pincode">Postal / Zip code<span style="color:red">*</span></label>
									<div class="row">
										<div class="col-md-12">
	...
	$.ajax({
		...
		//alert(data.discount);console.log(data)
			...
			if($.trim(data.response)=='1')
			{
				...
				$("#idtotamt").html($.trim(price)).show()//+$("#iddiscountamt").html((data.discount/100)*100).show();
				$("#payamt").html(($.trim(price)*data.discount)/100).show();
				//$("#payamt").html(data.discount/100).show();
				//$("#idpayamt").html($("#hdnOPrice").val()).show()
				 $("#iddiscountamt").html(data.discount).show()
			...
			//	$('#coModal p').html(dis).modal('show');
				$("#idtotamt").html($.trim(price)).show()+$("#iddiscountamt").html($("#hdnOPrice").val()-(($("#hdnOPrice").val()*data.discount)/100)).show() ;//final using
...
$('#btnChkoutPay').click(function(){
	alert('btnChkoutPay');
	$("#loading").show();
	$.ajax({
		type: "POST",
		//url: "<?php echo base_url('index.php/razorpay/checkout') ?>",
		data: {},
		success: function(result) {
			window.location.href= "<?php echo base_url();?>index.php/razorpay/checkout";
			$("#loading").hide();
		}
	});
}
