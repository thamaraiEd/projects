									...
									<div class="row">
										<div class="col-lg-4 col-md-4">
											<div class="form-group">
												<label for="ddlworktype">Work Type  <span style="color:red" class="mstar">*</span></label>
												<select  class="form-control input-sm" name="ddlworktype" id="ddlworktype">
													<option value="">Select</option> 
													<?php foreach($CompWorkType as $row)
													{
														if($userdata[0]['work_type']==$row['id'])
														{
														...
														echo "<option id='".$row['id']."' value='".$row['id']."' ".$selected5."  >".$row['type']."</option>";
													}
													?> 
												</select>	
											</div>
...
$("#frmaddactivity").validate({
	...
	{   
		if($("#frmaddactivity").valid())
		{
			...
			$.ajax({
			url: "<?php echo base_url(); ?>index.php/comp/update_activity",
			type:"POST",
			dataType:"json",
			data:formData,
			contentType: false,       
			...
			{ 
				...
				{
					 window.location="<?php echo base_url(); ?>index.php/comp/activitylist"
				}
				else
				{ 
