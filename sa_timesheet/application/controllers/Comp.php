...
/* Animator Reviewer ---------*/	
	public function activitylist()  
	{
		...
		$rowid='';
		$data['activitylist']=$this->Dashboard_model->getCompActivityList($this->session->user_id,$rowid);
		$this->load->view('header');
		$this->load->view('comp/activitylist',$data);
	...
	{ 
		...
		$data['CompWorkType']=$this->Dashboard_model->getCompWorkType();
		
		$this->load->view('header');
		$this->load->view('comp/add_activity',$data);
		$this->load->view('footer');
	}
	...
	{ 
		...
		$data['rowid']=$rowid;
		$data['userdata']=$this->Dashboard_model->getCompActivityList($this->session->user_id,$rowid);
		$data['CompWorkType']=$this->Dashboard_model->getCompWorkType();
		
		$this->load->view('header');
		$this->load->view('comp/edit_activity',$data);
		$this->load->view('footer');
	}
	...
	{ 	
		...
		if($istime==1)
		{
			$insertactivity=$this->Dashboard_model->Insert_CompActivity($this->session->user_id,$curDate,$start_time,$end_time,$txtprojectname,$txttaskname,$txttaskduration,$txtremarks,$curDateTime,$ddlworktype);
			
			$RecordType=$this->Dashboard_model->getCompTotalRecordInserted($this->session->user_id,$curDate); 
	...
	{ 	
		...
		if($istime==1)
		{
			$insertactivity=$this->Dashboard_model->Update_CompActivity($hdnrowid,$this->session->user_id,$curDate,$start_time,$end_time,$txtprojectname,$txttaskname,$txttaskduration,$txtremarks,$curDateTime,$ddlworktype);   
			$arrResult=array("response"=>"1","msg"=>"Activity ".$RecordType[0]['TotalRecord']." Created Successfully"); 
		}
		else
		{
