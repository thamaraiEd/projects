...
/* Animator Reviewer ---------*/	
	public function activitylist()  
	{
		...
		$rowid='';
		$data['activitylist']=$this->Dashboard_model->getStoryboardActivityList($this->session->user_id,$rowid);
		$this->load->view('header');
		$this->load->view('storyboard/activitylist',$data);
	...
	{ 
		...
		$this->load->view('header');
		$this->load->view('storyboard/add_activity');
		$this->load->view('footer');
	}
	...
	{ 
		...
		$data['rowid']=$rowid;
		$data['userdata']=$this->Dashboard_model->getStoryboardActivityList($this->session->user_id,$rowid);
		
		$this->load->view('header');
		$this->load->view('storyboard/edit_activity',$data);
		$this->load->view('footer');
	}
	...
	{ 	
		...
		if($istime==1)
		{
			$insertactivity=$this->Dashboard_model->Insert_StoryBoardActivity($this->session->user_id,$curDate,$start_time,$end_time,$txtprojectname,$txttaskname,$txttaskduration,$txtremarks,$curDateTime); 
			
			$RecordType=$this->Dashboard_model->getStoryboardTotalRecordInserted($this->session->user_id,$curDate); 
	...
	{ 	
		...
		if($istime==1)
		{
			$insertactivity=$this->Dashboard_model->Update_StoryboardActivity($hdnrowid,$this->session->user_id,$curDate,$start_time,$end_time,$txtprojectname,$txttaskname,$txttaskduration,$txtremarks,$curDateTime);   
			$arrResult=array("response"=>"1","msg"=>"Activity ".$RecordType[0]['TotalRecord']." Created Successfully"); 
		}
		else
		{
